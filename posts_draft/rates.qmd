...
The downside of them is that you need to use the rates function repeatedly times if you are going to compute rates for several regions.
...
## Practical example: General mortality rates in Brazilian states, from 2010 to 2021
...
To compute rates, we will need the state population size per age group each year. The `brpop` package present estimates for this. We just need to do some adaptations.
...
With the data ready, we can compute the direct adjusted rates with the `tidyrates` package.
...
To make some plots, let's make some modifications and pivot the data.
